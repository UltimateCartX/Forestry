...
import com.example.forestry.data.database.mapper.toEntity
import com.example.forestry.data.database.mapper.toModel
...
class ForestryRepository(
    ...
    private val TAG = "ForestryRepository"

    private fun ensureBearer(token: String) = if (token.startsWith("Bearer ")) token else "Bearer $token"
    ...
    suspend fun getMe(token: String): User = withContext(ioDispatcher) {
        api.getCurrentUser(ensureBearer(token))
    }

    suspend fun getTrees(token: String): List<Tree> = executeWithOfflineSupport(
        remoteCall = { api.getTrees(ensureBearer(token)) },
    ...
    suspend fun getProjects(token: String): List<Project> = executeWithOfflineSupport(
        remoteCall = { api.getProjects(ensureBearer(token)) },
        localCall = { dao.getAllProjects().map { it.toModel() } },
    ...
    suspend fun addProject(token: String, project: Project) = executeWithOfflineSupport(
        remoteCall = { api.addProject(ensureBearer(token), project) },
    ...
    suspend fun addTree(token: String, tree: Tree) = executeWithOfflineSupport(
        remoteCall = { api.addTree(ensureBearer(token), tree) },
    ...
    suspend fun syncData(token: String) = withContext(ioDispatcher) {
        ...
        dao.getAllProjects().forEach { entity ->
            try {
                api.addProject(bearerToken, entity.toModel())
        ...
        dao.getAllTrees().forEach { entity ->
            try {
                api.addTree(bearerToken, entity.toModel())
